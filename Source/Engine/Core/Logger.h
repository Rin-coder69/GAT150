...
namespace gaia {
	...
	public:
		...
		static void Log(LogLevel level, const std::string& message) {
			...
			if (static_cast<uint8_t>(s_enabledLevels & level) == static_cast<uint8_t>(LogLevel::None)) return;
			...
			case LogLevel::Info:
				prefix = "[INFO] ";
				color = "<use color codes>"; // Green
				break;
			case LogLevel::Warning:
				prefix = "[WARNING] ";
				color = "<use color codes>"; // Yellow
				break;
			case LogLevel::Error:
				prefix = "[ERROR] ";
				color = "<use color codes>"; // Red
				break;
			case LogLevel::Debug:
				prefix = "[DEBUG] ";
				color = "<use color codes>"; // Cyan
				break;
			default:
				prefix = "[UNKNOWN] ";
				color = "<use color codes>"; // Reset
				break;
			}
