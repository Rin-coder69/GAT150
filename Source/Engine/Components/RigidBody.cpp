...
namespace gaia {
	...
	void RigidBody::Read(const json::value_t& value) {
		...
		if (!shapeName.empty()) {
			if (gaia::tolower(shapeName) ==  "box") bodyDef.shape = PhysicsBody::Shape::Box;
			else if (gaia::tolower(shapeName) == "capsule") bodyDef.shape = PhysicsBody::Shape::Capsule;
			else if (gaia::tolower(shapeName) ==  "circle") bodyDef.shape = PhysicsBody::Shape::Circle;
		}
	}
}
